...
Global permissions are set to `allow` in `opencode.json`. Agents override specific permissions based on their role.

### Permission Philosophy

| Category | Agents | Rationale |
|----------|--------|-----------|
| **Full access** | metaburdz, go, zig, linux, nix, k8s, terraform | Primary orchestrator and infra/language agents need system paths, toolchains, external docs |
| **Research** | librarian | Can search web/code, but no editing or external filesystem (stays focused) |
| **Search-only** | explore | Internal codebase search only - no editing, no external access, no self-recovery loops |
| **Media analysis** | multimodal | Read-only analysis of images/PDFs - no editing, no bash, no external access |
| **Fast tasks** | quick | Simple tasks with guardrails - no doom loops, asks before external directory access |
| **Advisory** | principal, architect, security, sre | Expensive advisors - bash requires confirmation, external directory asks |
| **Implementation** | cicd, frontend, postgres, testing, perf, docs | Standard implementers - external directory asks, no web/code search (use librarian) |

### Permission Matrix

| Agent | edit | bash | task | external_directory | websearch | codesearch | doom_loop |
|-------|------|------|------|--------------------|-----------|------------|-----------|
| metaburdz | allow | allow | allow | allow | allow | allow | allow |
| go, zig, linux, nix, k8s, terraform | allow | allow | allow | allow | allow | allow | allow |
| librarian | deny | allow | deny | deny | allow | allow | allow |
| explore | deny | allow | deny | deny | deny | deny | deny |
| multimodal | deny | deny | deny | deny | deny | deny | allow |
| quick | allow | allow | allow | ask | deny | deny | deny |
| principal, security, sre | allow | ask | allow | ask | allow | allow | allow |
| architect | allow | deny | allow | ask | allow | allow | allow |
| cicd, frontend, postgres, testing, perf, docs | allow | allow | allow | ask | deny | deny | allow |
